...
class App:
    def __init__(self, master, queue):
        self.queue = queue
        self.master = master
        self.master.title("Aplikacja z ramką do wpisywania")
        ...
        self.label = ctk.CTkLabel(self.frame, text="Wprowadź tekst:")
        self.label.pack()

        self.input = ctk.CTkEntry(self.frame,width = 500)
        self.input.pack()

        self.button = ctk.CTkButton(self.frame, text="Wyślij", command=self.send_text)
        self.button.pack()
    ...
    def update_texts(self,text):
        ...
        if len(self.texts) > self.max_texts or sum([text.winfo_height() for text in self.texts]) > 500: 
...
You have to know for example how to know if i ask about case specific scenarios, based on the document.\
